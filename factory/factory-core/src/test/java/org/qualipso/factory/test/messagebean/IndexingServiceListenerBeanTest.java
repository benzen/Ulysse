...
public class IndexingServiceListenerBeanTest extends BaseSessionBeanFixture<IndexingServiceListenerBean> {

	private static Log logger = LogFactory.getLog(IndexingServiceListenerBeanTest.class);


	private static final Class[] usedBeans = {};

	public IndexingServiceListenerBeanTest() {
        ...
        try {
            mockery.checking(new Expectations() {
            {
                ...
                //the action is rethrow to the private queue
                 oneOf(connectionFactory).createConnection().createSession(true, javax.jms.Session.AUTO_ACKNOWLEDGE).createProducer(queue).send(with(aMessageWithProperties("index", "/fake/path", 3)));
                 inSequence(sequence1);
